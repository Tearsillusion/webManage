const register = (req, res, redisClient,sqlMain,moment) => {
	res.status(200)
	// email:邮箱--账号
	// code：验证码
	// nickName：昵称
	...
	if (!email) {
		...
		redisClient.getValue(email, async (code) => {
			...
			if (passCode === code) {
				...
				const insetData = await sqlMain.insetSql('user','account,nickName,password,create_time',[email, nickName, password,create_time])
